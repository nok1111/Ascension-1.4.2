...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"extrahealing", ITEM_PARSE_EXTRAHEALING },
};
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_SLOTTYPE: {
					...
					} else if (tmpStrValue == "rune1") {
						it.slotPosition |= SLOTP_RUNE1;
					} else if (tmpStrValue == "rune2") {
						it.slotPosition |= SLOTP_RUNE2;
					} else if (tmpStrValue == "rune3") {
						it.slotPosition |= SLOTP_RUNE3;
					} else {
				...
				case ITEM_PARSE_EXTRAHEALING: {
					abilities.specialSkills[SPECIALSKILL_EXTRAHEALING] = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
