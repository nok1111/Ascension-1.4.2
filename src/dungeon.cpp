...
void Dungeon::prepare(Player* player, uint8_t difficulty)
{
    ...
    if (require.party.max >= 2 && party) {
        ...
        if (party->getLeader() == player) {
            ...
            // Check the distance of the leader (player)
            if (!Position::areInRange<7, 7>(player->getPosition(), entrancePosition)) {
                player->sendFYIBox("You are too far from the dungeon entrance!");
                queue->onPlayerLeave(player, true);
                return;
            }

            // Distance check for all members
            for (Player* member : members) {
                if (!Position::areInRange<7, 7>(member->getPosition(), entrancePosition)) {
                    std::ostringstream text;
                    text << "Can't join dungeon, " << member->getName() << " is too far from the dungeon entrance!";
                    player->sendFYIBox(text.str());
                    queue->onPlayerLeave(player, true);
                    return;
                }
