...
class Zones
{
	public:
		...
		void addTile(Tile* tile) { tiles.insert(tile); }
		void removeTile(Tile* tile) { tiles.erase(tile); }
	...
	private:
		inline bool checkCreature(CreatureType_t dataType, const Creature* creature) const
		{
		...
		size_t getCount(CreatureType_t dataType) const
		{
			...
			for (auto& itTile : tiles)
			{
				const CreatureVector* tileCreatures = itTile->getCreatures();
				if (!tileCreatures)
				{
					continue;
				}
				switch (dataType)
				{
					...
					case CREATURETYPE_MONSTER:
					{
						...
						{
							if (itCreature && checkCreature(dataType, itCreature))
							{
								count++;
							}
						}
