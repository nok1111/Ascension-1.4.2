...
local strenghtfactor = player:getEffectiveSkillLevel(SKILL_SWORD) * 2

	local min = ((strenghtfactor) + (attack*0.12) + 2) / 1.5
	local max = ((strenghtfactor) + (attack*0.25) + 3) / 1.5
	return -min, -max
end
...
local function arcaneDamage(player, variant, target, missileEffectpos, animationroll, creature)
    ...
    if player and creature then
        ...
        Position(missileEffectpos):sendDistanceEffect(creature:getPosition(), CONST_ANI_FROSTFIRE_SWORD)
...
local function arcaneBolt(player, var, target, count)
    ...
    if player and creature then
        ...
        Position(playerpos):sendDistanceEffect(missileEffectpos, CONST_ANI_FROSTFIRE_SWORD)
...
function onUseWeapon(player, variant, creature)
  ...
  local magiclevel = player:getMagicLevel()
    ...
    local missileCount = minMissiles -- extra missile every 20 magic levels
    local count = 1
