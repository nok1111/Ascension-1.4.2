...
local mining_items = {
    {name = "Copper Ore", id = 20001, min = 2, max = 6, pouches = 2},
    {name = "Silver Ore", id = 20002, min = 2, max = 5, pouches = 3},
    {name = "Gold Ore", id = 20003, min = 1, max = 4, pouches = 4},
    {name = "Platinum Ore", id = 20004, min = 1, max = 2, pouches = 6},
}
...
function creatureSayCallback(cid, type, msg)
    ...
    if msgcontains(msg_l, "valuable pouches") then
        ...
        dialog = dialog .. "4. Platinum Ore ("..mining_items[4].pouches.." pouches)\n"
        dialog = dialog .. "Type 'copper', 'silver', 'gold', or 'platinum' to choose."
        doSendDialogNpc(cid, getNpcCid(), dialog, "copper&silver&gold&platinum&No")
    ...
    elseif msgcontains(msg_l, "platinum") then
        local item = mining_items[4]
        if player:getItemCount(valuable_pouch_id) >= item.pouches then
            player:removeItem(valuable_pouch_id, item.pouches)
            local amount = math.random(item.min, item.max)
            player:addItem(item.id, amount)
            doSendDialogNpc(cid, getNpcCid(), "Here's your Platinum Ore! Only for the most dedicated diggers.", "platinum&Close")
        else
            doSendDialogNpc(cid, getNpcCid(), "You don't have enough valuable pouches for this trade. Come back when you've got more!", "Close")
        end
