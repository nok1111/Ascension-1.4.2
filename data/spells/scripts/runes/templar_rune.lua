function removeArtifact(creatureId)
...
local function getSummonCount(player)
	...
	if #summons > 0 then
		for i = 1, #summons do
			summon = summons[i]
			if summon:getName():lower() == "holy artifact" or summon:getName():lower() == "holy artifact" then
...
for _, v in pairs(summons) do
	...
	local MyArtifactSummon = Game.createMonster(v[1], creaturepos, false, true)	
	creature:addSummon(MyArtifactSummon)
	MyArtifactSummon:sendProgressbar(5*1000, false)
    addEvent(removeArtifact, 5*1000, MyArtifactSummon:getId()) --I think this isn't necessary
       -- break
    end
end
